...
* `node` ([ from ][hast-element])
  — element to check
* `index` (`number | undefined`)
  — index of `element` in `parent`
* `parent` ([ from ][hast-node])
  — parent of `element`
...
* `node` ([ from ][hast-element], optional)
  — original node
...
* `remarkRehypeOptions` ([ from][remark-rehype-options], optional)
...
In this case, , which adds support for strikethrough,
...
We use  under the hood for our parsing.
...
unified does: please read through the  readme (the
...
**Note**: HTML in markdown is still bound by how [HTML works in][commonmark-html].
...
`ul`.
With , you can also use `del`, `input`, `table`,
...
This is the original [ from ][hast-element] element being turned
...
[api-options]: #option[api-url-transform]: #urltransform
